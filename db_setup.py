...
# Read the technical analysis file with the specified encoding
try:
...
# Ensure 'Position' column values are strings and handle missing values
technical_data['Position'] = technical_data['Position'].astype(str)

# Convert all drills, videos, and URLs to strings and handle missing values
...
# Expand the technical_data to have individual positions
...
for _, row in technical_data.iterrows():
    ...
    for pos in positions_list:
        expanded_technical_data.append({
            'Position': pos,
            'Technical Attributes': row['metric_name'],  # Use 'metric_name' as the technical attribute
            'Mean': row['mean'],
            'Drill 1': row['Drill 1'],
            'Video 1': row['Video 1'],
            'URL 1': row['URL 1'],
...
# Remove rows with NaN values in Technical Attributes or Mean
expanded_technical_df = expanded_technical_df.dropna(subset=['Technical Attributes', 'Mean'])

# Combine the physical and technical attributes
...
positions['id'] = positions.index + 1  # Create a unique ID for each position
positions.to_sql('positions', conn, if_exists='replace', index=False)

# Insert data into position_requirements table
...
# Insert data into position_details table
...
# Calculate and insert physical training data
...
for _, row in position_requirements.iterrows():
    ...
    physical_training_data.append({
        ...
        'exercises': 'Squats, Deadlifts, Bench Press, Power Cleans, Sprints, Push Press'  # Example exercises
    })
...
# Insert data into technical_drills table
technical_drills_data = []

for _, row in expanded_technical_df.iterrows():
    position_id = positions[positions['Position'] == row['Position']]['id'].values[0]  # Get position_id
